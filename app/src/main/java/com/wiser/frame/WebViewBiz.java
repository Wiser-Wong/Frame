...
public class WebViewBiz extends WISERBiz<WebViewActivity> implements IWebBiz {
	...
	@Override public void observableMethod() {
		...
		// }
		// }));
		httpObservableIO(http(IHttp.class).getPDF()).subscribe(httpDisposableObserver(new WISERRxJavaDisposableObserver<String>() {
		...
		// }));
		// /** zip事件合并 */
		// httpObservableIO(Observable.zip(http(IHttp.class).getObservableData(),
		// http(IHttp.class).getObservableData1(""),
		// http(IHttp.class).getObservableData(), http(IHttp.class).getObservableData(),
