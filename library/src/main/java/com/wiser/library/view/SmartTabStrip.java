...
class SmartTabStrip extends LinearLayout {
	...
	private final int						indicatorTopBottomPadding;
	...
	SmartTabStrip(Context context, AttributeSet attrs) {
		...
		indicatorTopBottomPadding = a.getDimensionPixelSize(R.styleable.stl_SmartTabLayout_stl_indicatorTopBottomPadding, 0);
	...
	private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness, int color) {
		...
		if (indicatorWidth == AUTO_WIDTH) {
			indicatorRectF.set(left, top - indicatorTopBottomPadding, right, bottom - indicatorTopBottomPadding);
		} else {
			float padding = (Math.abs(left - right) - indicatorWidth) / 2f;
			indicatorRectF.set(left + padding, top - indicatorTopBottomPadding, right - padding, bottom - indicatorTopBottomPadding);
		}
