...
public class WISERTextView {
	 ...
	 * @return
	 */
	public static SpannableString textformatSpan(String content, String subContent1, String subContent2, int color1, int color2, int count) {
		// 文本内容
		SpannableString ss;
		...
		// 设置下标处的字符颜色
		ss.setSpan(new ForegroundColorSpan(color1), index1, index1 + length1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		switch (count) {
			case 2:
				...
				ss.setSpan(new ForegroundColorSpan(color2), index2, index2 + length2, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
				break;
		}
	 ...
	 * @return
	 */
	public static void textformatSpan(TextView tv, String[] contents, int[] subIndexs, final int color, final int clickBgColor, final SpanClickCallBack clickCallBack) {
		if (tv == null) return;
		if (contents == null || subIndexs == null) {
		...
		for (int i = 0; i < contents.length; i++) {
			SpannableString ss = new SpannableString(contents[i]);
			for (int index : subIndexs) {
				if (index == i) {
					ss.setSpan(new ClickableSpan() {

						@Override public void onClick(@NonNull View widget) {
							if (clickCallBack != null) {
								clickCallBack.spanClick();
							}
						}
	 ...
	 *            （1为就一处颜色变更，2为两处颜色变更）
	 */
	public static void textInsertImageAndTextColorChange(Context context, List<Integer> imageIds, TextView text, String content, String subContent1, String subContent2, int color1, int color2,
			int count) {
		text.setText("");
		if (imageIds != null && imageIds.size() > 0) {
	...
	// 设置文本上面图片
	public static void setTextDrawable(Activity activity, TextView view, int drawable, String direction, int dp) {
		Drawable img = activity.getResources().getDrawable(drawable);
		// 调用setCompoundDrawables时，必须调用Drawable.setBounds()方法,否则图片不显示
		img.setBounds(0, 0, img.getMinimumWidth(), img.getMinimumHeight());
		view.setCompoundDrawablePadding(WISERApp.dip2px(dp));
		switch (direction) {
			case "left":
